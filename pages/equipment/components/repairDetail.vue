			...
			<view class="reportDetail" :style="{height: swiperWidth + 'px'}">
				...
				<view class="container">
					<uni-forms ref="form" :modelValue="formData" label-width="120px" label-position="top"
						:rules="rules">
						<uni-forms-item label="故障描述" name="failureDesc" class="upKeepContentItem">
							<textarea class="contentTextarea" v-model="formData.failureDesc" style="height: 72px;"
								placeholder-style="color:#aaa" disabled placeholder="请填写故障描述" />
						</uni-forms-item>
						<uni-forms-item label="润滑油、脂" name="lube" class="upKeepContentItem">
							<textarea class="contentTextarea" disabled v-model="formData.lube" placeholder-style="color:#aaa"
								placeholder="请填写润滑油、脂" />
						</uni-forms-item>
						<uni-forms-item label="机封" name="mechanicalSeal" class="upKeepContentItem">
							<textarea class="contentTextarea" v-model="formData.mechanicalSeal"
								placeholder-style="color:#aaa" disabled placeholder="请填写机封" />
						</uni-forms-item>
						<uni-forms-item label="轴承" name="bearing" class="upKeepContentItem">
							<textarea class="contentTextarea" v-model="formData.bearing"
								placeholder-style="color:#aaa" disabled placeholder="请填写轴承" />
						</uni-forms-item>
						<uni-forms-item label="其他" name="other" class="upKeepContentItem">
							<textarea class="contentTextarea" disabled v-model="formData.other" placeholder-style="color:#aaa"
								placeholder="请填写其他" />
						</uni-forms-item>
					</uni-forms>
...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			this.planUpkeepUserName = options.planUpkeepUserName
			this.upkeepTime = options.upkeepTime
			this.currentTaskName = options.name
			this.imageIds = options.imageIds
			this.formData.id = options.id
			this.formData.failureDesc = decodeURIComponent(options.failureDesc)
			this.formData.lube = decodeURIComponent(options.lube)
			this.formData.mechanicalSeal = decodeURIComponent(options.mechanicalSeal)
			this.formData.bearing = decodeURIComponent(options.bearing)
			this.formData.other = decodeURIComponent(options.other)
			this.formData.imageIds = options.imageIds
		...
		methods: {
			...
			acceptApprove(type){
				let params = {
				    ...
				    taskHttp.equipRepairBatchAcceptance(params).then(res => {
