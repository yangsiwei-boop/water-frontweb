			...
			<scroll-view scroll-y="true" :show-scrollbar='false'>
								...
								<view class="detail">
									...
									<view class="content">
										<view class="name" style="width: 75%;color: #ff575c">
											{{item.upkeepLevel == '1' ? '一级保养' : (item.upkeepLevel == '2' ? '二级保养' : '三级保养')}}
										</view>
									</view>
									<view class="content">
										<view class="">
											保养时间：{{item.upkeepTime}}
										</view>
										<view class="">
											保养人： {{item.upkeepUser}}
										</view>
				...
				<view class="inspectionEmpty" v-else>
					<view style="display: block;text-align: center;padding-top: 40px;">
						...
						<view style="color: #666;">无保养记录</view>
					</view>
				</view>
			</scroll-view>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			eqRepairDetail(item) {
				let upkeepName = item.upkeepLevel == '1' ? '一级保养' : (item.upkeepLevel == '2' ? '二级保养' : '三级保养')
				uni.navigateTo({
					url: `/pages/equipment/components/upkeepRecordDetail?upkeepName=${upkeepName}&upkeepTime=${item.upkeepTime}&upkeepUser=${item.upkeepUser}&imageIds=${item.imageIds}&items=${JSON.stringify(item.items)}`,
