				...
				<view class="container">
					<uni-forms ref="form" :modelValue="formData" label-width="120px" label-position="top"
						...
						<uni-forms-item label="故障描述" required name="failureDesc" class="upKeepContentItem">
							<textarea class="contentTextarea" v-model="formData.failureDesc" style="height: 72px;"
								placeholder-style="color:#aaa" placeholder="请填写故障描述" disabled />
						</uni-forms-item>
					</uni-forms>
					<view class="upKeepContentItem">
						...
						<view class="photo">
							...
							<!-- {{item.fileList}} -->
							<view class="takePhoto" v-show="fileList.length == 0">
					...
					<view class="upKeepContentItem">
						...
						<uni-data-select v-model="adjustType" :localdata="adjustList"></uni-data-select>
					</view>
					<view class="upKeepContentItem" v-show="adjustType =='确认故障'">
						<view class="title">
							分配维修人
						</view>
						<uni-data-select v-model="inspectionUser" :localdata="userList"></uni-data-select>
					</view>
					<view style="color: #999;" v-show="adjustType =='非故障'">
						<uni-icons type="info-filled" size="18"></uni-icons>
						确认【非故障】将关闭维修任务
					</view>
					<view style="color: #999;" v-show="adjustType =='报废'">
						<uni-icons type="info-filled" size="18"></uni-icons>
...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			this.formData.failureDesc = options.failureDesc
			this.formData.imageIds = options.failureImageIds
			this.formData.id = options.id
			this.formData.depId = options.depId
			this.formData.equipmentId = options.equipId
		},
		mounted() {
			if (this.formData.imageIds != 'null' && this.formData.imageIds != 'undefined' && this.formData.imageIds !=
				' ') {
				let imageIdList = this.formData.imageIds.split(',')
				let self = this
		...
		methods: {
			userQueryUserList() {
				...
				taskHttp.userQueryUserList(params).then(res => {
					...
					if (res.data.code == 200) {
						...
						self.userList = res.data.data.records;
					}
				})
			},
			...
			submit(form) {
				...
				if (this.adjustType == '确认故障' && this.inspectionUser == '') {
					uni.showToast({
						title: "请分配维修人",
						icon: "none",
					});
				...
				params.processingResult = this.adjustType;
				params.nextUser = this.inspectionUser;
				taskHttp.equipRepairProcessing(params).then(res => {
