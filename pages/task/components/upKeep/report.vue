			...
			<view class="reportDetail" :style="{height: swiperWidth + 'px'}">
				...
				<view class="container">
					<uni-forms ref="form" :modelValue="formData" label-width="120px" label-position="top"
						:rules="rules">
						<uni-forms-item label="保养内容" required name="upkeepDesc" class="upKeepContentItem">
							<textarea class="contentTextarea" v-model="formData.upkeepDesc" style="height: 72px;"
								placeholder-style="color:#aaa" placeholder="请填写保养内容" />
						</uni-forms-item>
						...
						<uni-forms-item label="密封件" required name="sealingElement" class="upKeepContentItem">
							<textarea class="contentTextarea" v-model="formData.sealingElement"
								placeholder-style="color:#aaa" placeholder="请填写密封件" />
						</uni-forms-item>
		...
		methods: {
			...
			submit(form) {
				this.$refs.form.validate().then(res => {
					...
					taskHttp.upkeepTaskReportTask(this.formData).then((res) => {
						if (res.data.code == "200") {
							uni.showToast({
								title: "填报成功",
								icon: "success",
